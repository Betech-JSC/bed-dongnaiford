...
export default function AccessoryDetailPage() {
  ...
  const accessory = accessoriesData.find(a => a.id === id) || null;
  ...
  if (id !== prevId) {
    setPrevId(id);
    setActiveImage(accessory ? accessory.images[0] : "");
    setQuantity(1);
  ...
  const relatedAccessories = accessoriesData
    .filter(a => a.id !== accessory.id && (a.category === accessory.category || a.category === "interior"))
    .slice(0, 4);
  ...
  if (relatedAccessories.length < 4) {
    ...
    accessoriesData.forEach(a => {
      if (a.id !== accessory.id && !ids.has(a.id) && relatedAccessories.length < 4) {
  ...
  const thumbnails = [...accessory.images];
  while (thumbnails.length < 5) {
    thumbnails.push(accessory.images[0]);
  }
    ...
    <div className="bg-[#fafafa] min-h-screen text-[#1a1a1a] font-sans pb-0">
      ...
      <div className="max-w-[1440px] mx-auto px-4 xl:px-[144px] w-full pt-[32px] pb-[12px]">
        <div className="flex flex-wrap items-center gap-[13px] text-[12px] text-gray-500 font-medium font-['Ford_Antenna',sans-serif]">
          ...
          <Link href="/accessories" className="hover:text-[#0562D2] transition-colors">
            Phụ Kiện
          </Link>
          <span className="w-[3px] h-[3px] rounded-full bg-[#333] opacity-60 shrink-0" />
