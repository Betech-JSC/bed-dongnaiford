...
import { jobsAPI, contactsAPI } from "@/lib/api";
...
export default function JobDetailPage({
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        const res = (await jobsAPI.getBySlug(slug)) as any;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#fafafa] flex flex-col items-center justify-center py-20">
        ...
        <p className="text-gray-500 font-medium font-['Ford_Antenna',sans-serif]">Đang tải chi tiết tuyển dụng...</p>
      </div>
    );
  }
  ...
  if (error || !job) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 py-16 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2 font-display">Không tìm thấy vị trí tuyển dụng</h2>
        <p className="text-gray-600 mb-6">Mẫu tin tuyển dụng bạn đang tìm kiếm không tồn tại hoặc đã hết thời hạn.</p>
        <Link
          href="/tuyen-dung"
          className="bg-[#0562d2] hover:bg-[#044ea7] text-white px-6 py-2.5 rounded-full font-semibold transition-colors cursor-pointer flex items-center gap-1"
        >
